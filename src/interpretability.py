"""
SENTINEL-DoH — Interpretability Module
========================================
Section VI of the specification.

* **SHAP** (TreeExplainer)  → Summary Plot + Force Plot for XGBoost.
* **Grad-CAM 1D**          → Heatmap of most contributing feature segments for CNN.
* **High-Confidence Failure** analysis is handled in evaluation.py.
"""
...
# =============================================================================
# SHAP for XGBoost (Section VI — C3)
# =============================================================================
def explain_xgboost(
    model,
    X_test: np.ndarray,
    ...
    2. **Force Plot** (HTML) — explanation for a single test sample.
    """
    import shap
    ...
    # ── Force Plot for a single malicious sample ─────────────────────────
    try:
        ...
        logger.info("SHAP force plot saved → %s", path)
    except Exception as e:
        logger.warning("Could not generate SHAP force plot: %s", e)


# =============================================================================
# Grad-CAM 1D for CNN (Section VI — C3) — PyTorch
# =============================================================================
def grad_cam_1d(
    model,
    X_sample: np.ndarray,
    ...
    # Register hook on conv2 to capture activations & gradients
    activations = {}
    gradients = {}
    ...
    # Forward pass (enable grad for input-independent gradient flow)
    x_input = x.clone().requires_grad_(True)
    logits = model(x_input)
    prob = torch.sigmoid(logits)

    # Backward on the positive-class probability
    model.zero_grad()
    prob.backward()
...
def explain_cnn(
    ...
    Produces heatmap overlays showing which feature positions contributed
    ...
    for ax, idx in zip(axes, mal_indices):
        ...
        # Interpolate heatmap to match feature count (after pooling, may differ)
    ...
    plt.savefig(path, dpi=150, bbox_inches="tight")
    plt.close()
    logger.info("Grad-CAM 1D plot saved → %s", path)
